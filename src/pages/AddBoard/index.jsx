...
import BookCreateModal from '@/components/BookCreateModal'; // ← 도서 모달 import

// ----------------- 회원 리스트 -----------------
function ShowMemberList({ refetchKey }) {
  const [dataMembers, setDataMembers] = useState([
    { studentId: 'Loading...', name: '로딩중...' },
  ]);
  ...
  const members = dataMembers.map((member) => (
    <li key={member.studentId} className={styles.memberItem}>
      ...
      <button
        ...
        disabled={member.studentId === 'Loading...'}
      >
        X
      </button>
    </li>
  ...
  return <ul className={styles.memberList}>{members}</ul>;
}
...
function ShowBookList({ refetchKey }) {
  ...
  const books = dataBooks.map((book) => (
    <li key={book.id} className={styles.bookItem}>
      <div className={styles.bookimage}>
        <img
          src={book.coverUrl || 'example_book.png'}
  ...
  return <ul className={styles.bookList}>{books}</ul>;
}
      ...
      <div className={styles.container}>
        ...
        <div className={styles.box}>
          <header className={styles.barHeader}>
            ...
            <button
              className={styles.barHeaderButton}
